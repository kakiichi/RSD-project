function tau_eff(vel,r_perp)
  use array
  implicit none
  real*8 :: tau_eff, vel, r_perp
  real*8 :: xi_v
  integer :: i,j
  ! quadpack parameters
  ...
  do i=1,N
     ...
     xi_array(i)=xi_v(v_array(i),r_perp)
  end do
  ...
  do j=1,M
     ...
     dtau_array(j)=dtau(vel,r_perp,NHI_array(j))
  end do
...
function dtau(vel,r_perp,NHI)
  use param
  use array
  implicit none
  real*8 :: dtau, vel, r_perp, NHI
  ! quadpack parameters
  real*8, parameter :: epsabs=0.0d0
  real*8, parameter :: epsrel=1.d-6
  real*8 :: result,abserr
  ...
  real*8 :: vel_share, r_perp_share, NHI_share
  common /share_dtau/ vel_share, r_perp_share, NHI_share
