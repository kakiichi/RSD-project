...
title=range(nfiles)
title[0]='REF'
title[1]='STRONG OUTFLOW'
title[2]='MODERATE OUTFLOW'
title[3]='WEAK OUTFLOW'
...
for n in range(nfiles):
    ...
    if n==3: 
        plot(s,xi_0,'g-.', linewidth=2,label=title[n])
    xlim(0,s_max)
    ylim(0,0.8)
    ...
    ylabel('$\\langle F_\\alpha(s)\\rangle_2$',fontsize=22)
    xlabel('$s$ $[\\rm pMpc]$',fontsize=22)
    if n==0: 
        #plot(s[1:],xi_2[1:]/xi_0[1:],'k-', linewidth=2,label=title[n])
        plot(s[1:],xi_2[1:],'k-', linewidth=2,label=title[n])
    if n==1: 
        #plot(s[1:],xi_2[1:]/xi_0[1:],'r--', linewidth=2,label=title[n])
        plot(s[1:],xi_2[1:],'r--', linewidth=2,label=title[n])
    if n==2: 
        #plot(s[1:],xi_2[1:]/xi_0[1:],'b:', linewidth=2,label=title[n])
        plot(s[1:],xi_2[1:],'b:', linewidth=2,label=title[n])
    if n==3: 
        #plot(s[1:],xi_2[1:]/xi_0[1:],'b:', linewidth=2,label=title[n])
        plot(s[1:],xi_2[1:],'g-.', linewidth=2,label=title[n])
    #ylim(-0.55,0.15)
    xlim(0,3)
